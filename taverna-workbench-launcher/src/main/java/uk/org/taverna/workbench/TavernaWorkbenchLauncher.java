...
public class TavernaWorkbenchLauncher {
	...
	private void launch() {
		try {
			...
			setDerbyPaths();
			OsgiLauncher osgilauncher = new OsgiLauncher(getAppDirectory(),
					getBundleURIs());
			...
			osgilauncher.startBundle(osgilauncher.installBundle(workbenchBundle
					.toURI()));
...
//			}
	...
	private void setFrameworkConfiguration(OsgiLauncher osgilauncher) {
		ApplicationProfile applicationProfile = applicationConfiguration
	...
	private List<URI> getBundleURIs() {
		List<URI> bundleURIs = new ArrayList<>();
		ApplicationProfile applicationProfile = applicationConfiguration
				.getApplicationProfile();
	...
	private void setDerbyPaths() {
		System.setProperty("derby.system.home", getAppDirectory()
				.getAbsolutePath());
		File logFile = new File(applicationConfiguration.getLogDir(),
				"derby.log");
